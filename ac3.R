...
# ac3_fit() function creates model variables of extended actor-critic model 
...
# ac3() function performs model fitting using maximum likelihood estimation 
# in addition to a penalization term
ac3 = function (data, median.alpha.critic, penalty.alpha.critic, 
                median.alpha.actor.ch, penalty.alpha.actor.ch, 
                median.alpha.actor.nc, penalty.alpha.actor.nc, 
  ...
  par = optim(
    ...
    function (par, data, median.alpha.critic, median.alpha.actor.ch, 
              median.alpha.actor.nc, median.beta) {
      alpha.critic = par[1]
...
ac3_and_feat = function (subject, median.alpha.critic, penalty.alpha.critic, 
                         median.alpha.actor.ch, penalty.alpha.actor.ch, 
                         median.alpha.actor.nc, penalty.alpha.actor.nc, 
                         median.beta, penalty.beta) {
  behav_raw = read_behav(paste("behavioral/", subject, "_RLT.txt", sep = ""))
  behav = ac3(behav_raw, median.alpha.critic, penalty.alpha.critic, 
              median.alpha.actor.ch, penalty.alpha.actor.ch,
              median.alpha.actor.nc, penalty.alpha.actor.nc, 
              median.beta, penalty.beta)
  cat("\n")
    ...
    for (penalty.alpha.actor.ch in 2*(1:5) / 10) { 
      for (penalty.alpha.actor.nc in 2*(1:5) / 10) { 
        for (penalty.beta in 2*(1:5) / 10) { 
          ...
          for (subject in train) {
            y_train = rbind(y_train, ac3_and_feat(subject, median.alpha.critic, 
                                                  penalty.alpha.critic, median.alpha.actor.ch, 
                                                  penalty.alpha.actor.ch, median.alpha.actor.nc, 
                                                  penalty.alpha.actor.nc, median.beta, penalty.beta))
          print(y_train)
            }
          for (subject in test) {
            y_test = rbind(y_test, ac3_and_feat(subject, median.alpha.critic, 
                                                penalty.alpha.critic, median.alpha.actor.ch, 
                                                penalty.alpha.actor.ch, median.alpha.actor.nc, 
                                                penalty.alpha.actor.nc, median.beta, penalty.beta))
          print(y_test)
          ...
          for (roi in rois) {
            for (what in c("V", "rpe", "abs_rpe", "pos_rpe", "neg_rpe", "rpe_nc", "abs_rpe_nc", 
                           "pos_rpe_nc", "neg_rpe_nc")) {
