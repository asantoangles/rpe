...
# ac1() function performs model fitting using maximum likelihood estimation in addition to a penalization term
ac1 = function (data, median.alpha.critic, penalty.alpha.critic, median.alpha.actor, penalty.alpha.actor, median.beta, penalty.beta) {
...
ac1_and_feat = function (subject, median.alpha.critic, penalty.alpha.critic, median.alpha.actor, penalty.alpha.actor, median.beta, penalty.beta) {
  behav_raw = read_behav(paste("behavioral/", subject, "_RLT.txt", sep = ""))
  behav = ac1(behav_raw, median.alpha.critic, penalty.alpha.critic, median.alpha.actor, penalty.alpha.actor, median.beta, penalty.beta)
  cat("\n")
  ...
  for (penalty.alpha.critic in 2*(1:5) / 10) { 
    for (penalty.alpha.actor in 2*(1:5) / 10) { 
      for (penalty.beta in 2*(1:5) / 10) { 
        ...
        for (subject in train) {
          y_train = rbind(y_train, ac1_and_feat(subject, median.alpha.critic, penalty.alpha.critic, median.alpha.actor, penalty.alpha.actor, median.beta, penalty.beta))
        }
        for (subject in test) {
          y_test = rbind(y_test, ac1_and_feat(subject, median.alpha.critic, penalty.alpha.critic, median.alpha.actor, penalty.alpha.actor, median.beta, penalty.beta))
        }
        for (roi in rois) {
