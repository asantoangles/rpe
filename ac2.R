...
call_feat = function (time, duration, value) {
  write.table(data.frame(time, duration, value), "faked_feat/ac2/rpe.txt",
              col.names = FALSE, row.names = FALSE)
  system("rm -rf faked_feat/ac2/result*.feat")
  system("feat faked_feat/ac2/faked_feat.fsf", ignore.stdout = TRUE)
  X = read.table("faked_feat/ac2/result.feat/design.mat", skip = 5)
  colnames(X) = c("x", "t")
  X
}
...
# ac2_fit() function creates model variables of extended actor-critic model assuming alpha.actor.ch = alpha.actor.nc (ac2)
ac2_fit = function (data, alpha.critic, alpha.actor, beta) {
  data$V = NA 
  data$w0 = NA 
  data$w1 = NA 
  data$p0 = NA
  ...
  for (stimulus in unique(data$stimulus)) {
    ...
    for (row in rows) {
      ...
      if (data$resp[row] == 0) {
        w0 = w0 + alpha.actor * delta.ch
        w1 = w1 + alpha.actor * delta.nc
      } else {
        w1 = w1 + alpha.actor * delta.ch
        w0 = w0 + alpha.actor * delta.nc
      }
...
# ac2() function performs model fitting using maximum likelihood estimation in addition to a penalization term
ac2 = function (data, median.alpha.critic, penalty.alpha.critic, median.alpha.actor, penalty.alpha.actor, median.beta, penalty.beta) {
  ...
  par = optim(
    ...
    function (par, data, median.alpha.critic, median.alpha.actor, median.beta) {
      ...
      cat(".")
      data.fitted = ac2_fit(data, alpha.critic, alpha.actor, beta)
      -(
  ...
  list(
    ...
    data = ac2_fit(data, par[1], par[2], par[3])
  )
}

# ac2_and_feat() function combines previous functions to perform model fitting and linear regression 
ac2_and_feat = function (subject, median.alpha.critic, penalty.alpha.critic, median.alpha.actor, penalty.alpha.actor, median.beta, penalty.beta) {
  behav_raw = read_behav(paste("behavioral/", subject, "_RLT.txt", sep = ""))
  behav = ac2(behav_raw, median.alpha.critic, penalty.alpha.critic, median.alpha.actor, penalty.alpha.actor, median.beta, penalty.beta)
  cat("\n")
...
for (fold in 1:10) {
  ...
  for (subject in train) {
    behav_raw = read_behav(paste("behavioral/", subject, "_RLT.txt", sep = ""))
    behav = ac2(behav_raw, 0.5, 0, 0.5, 0, 1, 0)
  ...
  for (penalty.alpha.critic in 2*(1:5) / 10) { 
    for (penalty.alpha.actor in 2*(1:5) / 10) { 
      for (penalty.beta in 2*(1:5) / 10) { 
        ...
        for (subject in train) {
          y_train = rbind(y_train, ac2_and_feat(subject, median.alpha.critic, penalty.alpha.critic, median.alpha.actor, penalty.alpha.actor, median.beta, penalty.beta))
        }
        for (subject in test) {
          y_test = rbind(y_test, ac2_and_feat(subject, median.alpha.critic, penalty.alpha.critic, median.alpha.actor, penalty.alpha.actor, median.beta, penalty.beta))
        }
        for (roi in rois) {
  ...
  write.table(X, paste("fold", fold, "_ac2.txt", sep = ""), quote = FALSE, row.names = FALSE, sep = "\t")
}