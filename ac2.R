...
# ac2_fit() function creates model variables of extended actor-critic model 
# assuming alpha.actor.ch = alpha.actor.nc (ac2)
...
# ac2() function performs model fitting using maximum likelihood estimation 
# in addition to a penalization term
ac2 = function (data, median.alpha.critic, penalty.alpha.critic, median.alpha.actor, 
...
ac2_and_feat = function (subject, median.alpha.critic, penalty.alpha.critic, 
                         median.alpha.actor, penalty.alpha.actor, 
                         median.beta, penalty.beta) {
  behav_raw = read_behav(paste("behavioral/", subject, "_RLT.txt", sep = ""))
  behav = ac2(behav_raw, median.alpha.critic, penalty.alpha.critic, median.alpha.actor, 
              penalty.alpha.actor, median.beta, penalty.beta)
  cat("\n")
...
for (fold in 1:10) {
  ...
  X = expand.grid(
    fold = fold, roi = rois, what = c("V", "rpe", "abs_rpe", "pos_rpe", 
                                      "neg_rpe", "rpe_nc", "abs_rpe_nc", 
                                      "pos_rpe_nc", "neg_rpe_nc"), max_z = 0,
  ...
  for (penalty.alpha.critic in 2*(1:5) / 10) { 
    for (penalty.alpha.actor in 2*(1:5) / 10) { 
      for (penalty.beta in 2*(1:5) / 10) { 
        ...
        for (subject in train) {
          y_train = rbind(y_train, ac2_and_feat(subject, median.alpha.critic, 
                                                penalty.alpha.critic, median.alpha.actor, 
                                                penalty.alpha.actor, median.beta, penalty.beta))
        }
        for (subject in test) {
          y_test = rbind(y_test, ac2_and_feat(subject, median.alpha.critic, 
                                              penalty.alpha.critic, median.alpha.actor, 
                                              penalty.alpha.actor, median.beta, penalty.beta))
        }
        for (roi in rois) {
          for (what in c("V", "rpe", "abs_rpe", "pos_rpe", "neg_rpe", 
                         "rpe_nc", "abs_rpe_nc", "pos_rpe_nc", "neg_rpe_nc")) {
