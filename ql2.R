...
# ql2_fit() function creates model variables of extended Q-learning model 
# assuming alpha_chosen = alpha_nochosen (ql2)
...
# ql2() function performs model fitting using maximum likelihood estimation
...
for (fold in 1:10) {
  ...
  for (penalty.alpha in 2*(1:5) / 10) { 
    for (penalty.beta in 2*(1:5) / 10) { 
      ...
      for (subject in train) {
        y_train = rbind(y_train, ql2_and_feat(subject, median.alpha, 
                                              penalty.alpha, median.beta, penalty.beta))
      }
      for (subject in test) {
        y_test = rbind(y_test, ql2_and_feat(subject, median.alpha, 
                                            penalty.alpha, median.beta, penalty.beta))
      }
      for (roi in rois) {
