...
call_feat = function (time, duration, value) {
  write.table(data.frame(time, duration, value), "faked_feat/ql2/rpe.txt",
              col.names = FALSE, row.names = FALSE)
  system("rm -rf faked_feat/ql2/result*.feat")
  system("feat faked_feat/ql2/faked_feat.fsf", ignore.stdout = TRUE)
  X = read.table("faked_feat/ql2/result.feat/design.mat", skip = 5)
  colnames(X) = c("x", "t")
  X
}
...
# ql2_fit() function creates model variables of extended Q-learning model assuming alpha_chosen = alpha_nochosen (ql2)
ql2_fit = function (data, alpha, beta) {
  data$ev0 = NA
  data$ev1 = NA
  data$p0 = NA
  data$p1 = NA
  ...
  for (stimulus in unique(data$stimulus)) {
    ...
    for (row in rows) {
      ...
      if (data$resp[row] == 0) {
        ...
        ev0 = ev0 + alpha * delta_ch
        ev1 = ev1 + alpha * delta_nc
      } else {
        delta_ch = data$correct[row] - ev1
        delta_nc = (1 - data$correct[row]) - ev0
        ev1 = ev1 + alpha * delta_ch
        ev0 = ev0 + alpha * delta_nc
      }
...
# ql2() function performs model fitting using maximum likelihood estimation in addition to a penalization term
ql2 = function (data, median.alpha, penalty.alpha, median.beta, penalty.beta) {
  ...
  par = optim(
    ...
    function (par, data, median.alpha, median.beta) {
      ...
      cat(".")
      data.fitted = ql2_fit(data, alpha, beta)
      -(
  ...
  list(
    ...
    data = ql2_fit(data, par[1], par[2])
  )
}

# ql2_and_feat() function combines previous functions to perform model fitting and linear regression 
ql2_and_feat = function (subject, median.alpha, penalty.alpha, median.beta, penalty.beta) {
  behav_raw = read_behav(paste("behavioral/", subject, "_RLT.txt", sep = ""))
  behav = ql2(behav_raw, median.alpha, penalty.alpha, median.beta, penalty.beta)
  cat("\n")
...
for (fold in 1:10) {
  ...
  for (penalty.alpha in 2*(1:5) / 10) { 
    for (penalty.beta in 2*(1:5) / 10) { 
      ...
      for (subject in train) {
        y_train = rbind(y_train, ql2_and_feat(subject, median.alpha, penalty.alpha, median.beta, penalty.beta))
      }
      for (subject in test) {
        y_test = rbind(y_test, ql2_and_feat(subject, median.alpha, penalty.alpha, median.beta, penalty.beta))
      }
      for (roi in rois) {
  ...
  write.table(X, paste("fold", fold, "_ql2.txt", sep = ""), quote = FALSE, row.names = FALSE, sep = "\t")
}
