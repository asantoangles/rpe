...
# ql3_fit() function creates model variables of extended Q-learning model assuming alpha_chosen != alpha_nochosen (ql3)
...
# ql3() function performs model fitting using maximum likelihood estimation in addition to a penalization term
ql3 = function (data, median.alpha.ch, penalty.alpha.ch, median.alpha.nc, penalty.alpha.nc, median.beta, penalty.beta) {
...
ql3_and_feat = function (subject, median.alpha.ch, penalty.alpha.ch, median.alpha.nc, penalty.alpha.nc, median.beta, penalty.beta) {
  behav_raw = read_behav(paste("behavioral/", subject, "_RLT.txt", sep = ""))
  behav = ql3(behav_raw, median.alpha.ch, penalty.alpha.ch, median.alpha.nc, penalty.alpha.nc, median.beta, penalty.beta)
  cat("\n")
  ...
  for (penalty.alpha.ch in 2*(1:5) / 10) { 
    for (penalty.alpha.nc in 2*(1:5) / 10) { 
      for (penalty.beta in 2*(1:5) / 10) { 
        ...
        for (subject in train) {
          y_train = rbind(y_train, ql3_and_feat(subject, median.alpha.ch, penalty.alpha.ch, median.alpha.nc, penalty.alpha.nc, median.beta, penalty.beta))
        }
        for (subject in test) {
          y_test = rbind(y_test, ql3_and_feat(subject, median.alpha.ch, penalty.alpha.ch, median.alpha.nc, penalty.alpha.nc, median.beta, penalty.beta))
        }
        for (roi in rois) {
