...
# ql1() function performs model fitting using maximum likelihood estimation in addition to a penalization term
...
for (fold in 1:10) {
  ...
  for (penalty.alpha in 2*(1:5) / 10) { 
    for (penalty.beta in 2*(1:5) / 10) {
      ...
      for (subject in train) {
        y_train = rbind(y_train, ql1_and_feat(subject, median.alpha, penalty.alpha, median.beta, penalty.beta))
        }

      for (subject in test) {
      y_test = rbind(y_test, ql1_and_feat(subject, median.alpha, penalty.alpha, median.beta, penalty.beta))
      }
